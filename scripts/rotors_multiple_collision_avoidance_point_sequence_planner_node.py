...
class RotorSMultipleCollisionAvoidancePointSequencePlannerNode():
    ...
    def _generate_initial_waypoint(self):
        if self._pos[2] < 1.0:
            ...
            max_acc = 3.0
            max_vel = 0.7
            self._roaming_planner = ptg.PlannerToGoal(self._waypoint, pos_gain, vel_gain, max_acc, max_vel)
        else:
    ...
    def work(self):
        ...
        max_acc = 6.0
        max_vel = 1.0
        self._collision_avoidance_planner = pcam.PlannerCollisionAvoidanceMultiple(gain, ths, max_acc, max_vel)
        ...
        while not rospy.is_shutdown():
            ...
            ca_vel = self._collision_avoidance_planner.get_velocity(self._pos, others_poss, rospy.get_time())
            ...
            if numpy.linalg.norm(self._pos-self._waypoint) < 0.15:
                ...
                waypoint_counter += 1
                self._last_update = float(rospy.get_time())
                self._generate_waypoint()
            ...
            print("TIME FROM LAST WAYPOINT: " + str(rospy.get_time()-self._last_update))
...
if __name__ == '__main__':
    node = RotorSMultipleCollisionAvoidancePointSequencePlannerNode()
    node.work()
