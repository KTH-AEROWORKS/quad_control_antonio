...
LeePositionControllerNode::LeePositionControllerNode() {
  ...
  joint_1_velocity_reference_pub_ = nh.advertise<mav_msgs::Actuators>("/firefly/gazebo/command/joint_1_speed", 1);	// NEW LINE ADDED !
...
void LeePositionControllerNode::OdometryCallback(const nav_msgs::OdometryConstPtr& odometry_msg) {
  ...
  mav_msgs::ActuatorsPtr joint_1_msg(new mav_msgs::Actuators);
  ...
  joint_1_msg->angular_velocities.push_back(0.5);		//NEW LINE ADDED ! MOVE THE JOINT WITH VELOCITY 0.5 r/s
  joint_1_velocity_reference_pub_.publish(joint_1_msg);
}
